...
void solve(){
	...
	fo(i,n){
		// we will start suffix from index i here
		foa(j,i,n){
			if(j - i > 38){
				// number will go down to 0
				break;
			}
			suff[i] += a[j] / pow2[j - i + 1];
		}
	}
	ckmax(best, suff[0]);
	fo(i,n-1){
		ckmax(best, pref[i] - k * (i + 1) + suff[i+1]);
	}
	ckmax(best, pref[n-1] - k * n);
	ps(best);
}
